...
function default_1(page, total, count, url, pagetype) {
    if (pagetype === '') {
        if (total == 1)
            return '';
        let html = '';
        ...
        if (total > 0) {
            ...
            if (page == 1) {
                ...
                html += `<li><a class="arrow" href="${query}p=${helper_1.enc((page - 1) + '.' + count)}#${pagerid}"></a></li>`;
            }
            let pagelimit = 8;
            ...
            for (let i = start; i <= end; i++) {
                ...
                if (i == page) {
                    ...
                    html += `<li><a href="${query}p=${helper_1.enc(i + '.' + count)}#${pagerid}">${i}</a></li>`;
                }
                if (i == end && i < total)
                    html += '<li class="disabled"><a>...</a></li>';
            }
            if (page == total) {
                ...
                html += `<li><a class="arrow next" href="${query}p=${helper_1.enc((page + 1) + '.' + count)}#${pagerid}"></a></li>`;
            }
            html += '</ul>';
        }
        return html;
